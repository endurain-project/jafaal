...
def test_change_managed_user_password_revokes_all_sessions(db, make_user):
    user = make_user(password="Old1!Pass")
    session_utils.create_session("s1", user, _request(), "rt", password_hasher, db)
...
def test_delete_other_user_sessions(db, make_user):
    user = make_user()
    session_utils.create_session("keep", user, _request(), "a", password_hasher, db)
    session_utils.create_session("drop", user, _request(), "b", password_hasher, db)
...
def test_identity_service_delete_other_sessions_returns_count(db, make_user):
    ...
    user = make_user()
    session_utils.create_session("keep", user, _request(), "a", password_hasher, db)
    session_utils.create_session("drop-1", user, _request(), "b", password_hasher, db)
    session_utils.create_session("drop-2", user, _request(), "c", password_hasher, db)
