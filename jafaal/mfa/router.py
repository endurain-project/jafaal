...
def enable_mfa(
    request: Request,
    response: Response,
    payload: mfa_schema.MFASetupRequest,
    _check_scope: Annotated[None, Security(jafaal_dependencies.check_scopes, scopes=["profile"])],
    principal: Annotated[Principal, Depends(jafaal_dependencies.get_current_principal)],
...
def disable_mfa(
    request: Request,
    payload: mfa_schema.MFADisableRequest,
    _check_scope: Annotated[None, Security(jafaal_dependencies.check_scopes, scopes=["profile"])],
    principal: Annotated[Principal, Depends(jafaal_dependencies.get_current_principal)],
    identity_service: Annotated[
        jafaal_identity_service.LocalCredentialStore,
...
def verify_mfa(
    ...
    return mfa_workflow.verify_mfa(payload, _user_id(principal), identity_service, db)
...
def generate_backup_codes(
    request: Request,
    response: Response,
    payload: jafaal_schema.StepUpVerification,
    _check_scope: Annotated[None, Security(jafaal_dependencies.check_scopes, scopes=["profile"])],
    principal: Annotated[Principal, Depends(jafaal_dependencies.get_current_principal)],
    identity_service: Annotated[
        jafaal_identity_service.LocalCredentialStore,
