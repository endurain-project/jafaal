...
- ``jafaal._internal.services.mfa_workflow`` (this module): the workflow facade profile
...
- ``jafaal.mfa.service``: pure TOTP/QR helpers plus the per-user state helpers
  (setup/enable/disable/verify) this module orchestrates.
- ``jafaal.mfa.crud``: persistence for the ``users_mfa`` table.

Profile routes depend on this module, never on ``jafaal.mfa.*`` directly.
"""
